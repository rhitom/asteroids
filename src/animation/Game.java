...
/**
 * Game class creates Spaceship, Asteroids, Saucers and Bullets. Runs the
 * animation using these objects, levels up, Checks for collisions, calculates
 * score and saves high scores.
 * 
...
//        String myLives = " "; //Arrays.toString(lives.toArray());
//        
...
//    }
    ...
    private void updateShip() {
        ...
        // If rotating left, calls rotateL()
        else if (rotatingL) { 
            spaceship.rotateL();
        }
    }
    ...
    private void checkSaucersCollisions() {
        ...
        for (int i = 0; i < asteroids.size(); i++) {
            for (int y = 0; y < saucers.size(); y++) {
                if (!saucers.get(y).collided && Saucer.checkAsteroidSaucerCollision(
                        asteroids.get(i), saucers.get(y))) {
                    saucers.get(y).collided = true;
                }
            }
        }

        // Checks for saucer-bullet collisions
        // Bullet disappears if it hits an object
        for (int i = 0; i < saucers.size(); i++) {
            for (int y = 0; y < shipBullets.size(); y++) {
                if (!(saucers.get(i).collided)
                        && !(shipBullets.get(y).collided)) {
                    if (Saucer.checkSaucersBulletCollision(saucers.get(i),
                            shipBullets.get(y))) {
    ...
    public void makeBullets() {
    	// buffer = 3 ensures we never shoot more than one bullet every 3 frames
        ...
        //TODO: comment/explain this code
        for (int i = 0; i < shipBullets.size(); i++) {
    ...
    private static double radiansBetweenTwoPoints(double x1, double y1,
            double x2, double y2) {
        return Math.atan2(y2 - y1, x2 - x1) + Math.PI / 2;
    }
    
    /**
     * Makes saucers shoot bullets by initializing then adding bullets to the saucers'
     * bullet arrayList. Saucers shoot in direction of our spaceship.
     */
    public void saucersMakeBullets() {
    	// shoot predefined amount of saucer-bullets in quick succession
        for (int i = 0; i < Game.SAUCER_SHOOT_AMOUNT; i++) {
            if (frames % (120 + 15 * i) == 0) {
                if (saucers.size() != 0) {
                    for (int y = 0; y < saucers.size(); y++) {
                    	//makes sure saucers can only shoot when in bounds
                        if (isInBounds(saucers.get(y).getX(),
                                Game.saucers.get(y).getY())
                                && !(saucers.get(y).collided)) {
                            // Creating a bullet object using the location of
                            // the saucer
                            saucerBullet = new Bullet(this,
