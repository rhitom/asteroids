...
/**
 * Creates animated object Asteroid, which spawns at a random location on the
 * borders of the window, moves in straight line, destroys ship if hits it, and
 * is destroyed by bullet.
 * 
...
public class Asteroid implements AnimatedObject {
    ...
    public Asteroid(AbstractAnimation animation) {
        ...
        // Create the shape of the animation
    ...
    /**
     * Randomly generates a number (either 1 or 2) to 
     * set the color of the asteroid to grey or yellow
     */
    static int setRandomColor() {
        Random rand = new Random();
        return rand.nextInt(2);
    }

    /**
     * Paints the asteroid with a "skin" on top
     * 
    ...
    public void paint(Graphics2D g) {
        ...
        case 0:
            image_path = "asteroid_grey.PNG";
            break;
        // Importing the yellow asteroid
        case 1:
            image_path = "asteroid_yellow.PNG";
            break;
        }
        ;
    ...
    public static boolean checkAsteroidsBulletCollision(Asteroid asteroid, Bullet bullet) {
        return !asteroid.collided && !bullet.collided && (asteroid.getShape().intersects(bullet.getShape().getBounds2D()));
    }
     ...
     * 
     * @return result, a number that will determine the 
     *         direction of the asteroid
     */
    static double randomMoveGenerator() {
        Random rand = new Random();
    ...
    /**
     * Helper method that ensures that the asteroids spawn on random sides of
     * the screen
     * 
     * @return coord, a random coordinate on one of the borders
     */
    private ArrayList<Integer> randomCoordOnBorder() {
        // The arrayList that will hold our coordinates
        ...
        case 0:
            ...
            break;
        case 1:
            coord.add(rand.nextInt(width) + ASTEROID_SIZE / 2);
            coord.add(0 - ASTEROID_SIZE / 2);
            break;
        case 2:
            coord.add(width + ASTEROID_SIZE / 2);
            coord.add(rand.nextInt(height) + ASTEROID_SIZE / 2);
            break;
        case 3:
            coord.add(rand.nextInt(width) + ASTEROID_SIZE / 2);
    ...
    /**
     * Moves the asteroid a small amount. If it reaches the edges, 
     * it wraps to the other side of the screen
     */
